    ...
    def run(self, grid: Grid) -> Grid:
        ...
        for row in range(0, 9):
            for col in range(0, 9):
                ...
                candidates_in_same_row_column_and_grid = [
                    cells_on_same_row.get_candidates_union(),
                    cells_on_same_column.get_candidates_union(),
                    cells_in_same_grid.get_candidates_union(),
                ]

                for candidates_group in candidates_in_same_row_column_and_grid:
                    unique_candidates_in_group = candidates.difference(candidates_group)
                    if len(unique_candidates_in_group) == 1:
                        grid.set_cell_value(cell, list(unique_candidates_in_group)[0])
    ...
    def run(self, grid: Grid) -> Grid:
        for square in grid.get_all_squares():
            for row in square.get_rows():
                empty_cells_in_subrow = CellsGroup(
                ...
                unique_candidates_in_subrow = (
                    empty_cells_in_subrow.get_candidates_union().difference(
                        empty_cells_on_same_row_in_other_squares.get_candidates_union()
                    )
                )
                ...
                other_empty_cells_in_square = square.get_other_empty_cells_in_square(
                    empty_cells_in_subrow.get_cells()
                )
            ...
            for column in square.get_columns():
                empty_cells_in_subcolumn = CellsGroup(
                ...
                empty_cells_on_same_column_in_other_squares = (
                    grid.get_empty_cells_on_col(col_index).difference(
                        empty_cells_in_subcolumn
                    )
                )
                ...
                other_empty_cells_in_square = square.get_other_empty_cells_in_square(
                    empty_cells_in_subcolumn.get_cells()
                )
    ...
    def run(self, grid: Grid) -> Grid:
        for square in grid.get_all_squares():
            for row in square.get_rows():
                ...
                empty_cells_on_same_row_in_other_squares = CellsGroup(
                ...
                unique_candidates_in_subrow = (
                    empty_cells_in_subrow.get_candidates_union().difference(
                        other_empty_cells_in_square.get_candidates_union()
                    )
                )
            ...
            # same logic on columns
            for column in square.get_columns():
                empty_cells_in_subcol = CellsGroup(
                    {cell for cell in column.get_cells() if cell.is_empty()}
                )
                ...
                other_empty_cells_in_square = square.get_other_empty_cells_in_square(
                    empty_cells_in_subcol.get_cells()
                )

                empty_cells_on_same_col_in_other_squares = CellsGroup(
                ...
                unique_candidates_in_subcol = (
                    empty_cells_in_subcol.get_candidates_union().difference(
                        other_empty_cells_in_square.get_candidates_union()
                    )
                )
                ...
                for cell in empty_cells_on_same_col_in_other_squares.get_cells():
                    cell.remove_candidates(unique_candidates_in_subcol)
    ...
    def run(self, grid: Grid) -> Grid:
        for square in grid.get_all_squares():
            cells_with_2_candidates = [
                cell
                for cell in square.get_empty_cells()
                if len(cell.get_candidates()) == 2
            ]
            ...
            for cell_1 in cells_with_2_candidates:
                for cell_2 in cells_with_2_candidates:
                    if (
                        cell_1 == cell_2
                        or cell_1.get_candidates() != cell_2.get_candidates()
                    ):
                        continue
...
class DoubleCoupleAlignedTechnique(AbstractJob):
    ...
    def run(self, grid: Grid) -> Grid:
        for row in grid.get_rows():
            cells_with_2_candidates = [
                cell
                for cell in row.get_empty_cells()
                if len(cell.get_candidates()) == 2
            ]
        ...
        for col in grid.get_columns():
            cells_with_2_candidates = [
                cell
                for cell in col.get_empty_cells()
                if len(cell.get_candidates()) == 2
            ]
    ...
    def run(self, grid: Grid) -> Grid:
        ...
        for rcs_group in rcs_groups:
            for rcs in rcs_group:
                empty_cells_in_rcs = {cell for cell in rcs.get_empty_cells()}

                for cell_1 in empty_cells_in_rcs:
                    for cell_2 in empty_cells_in_rcs:
                        for cell_3 in empty_cells_in_rcs:
                            if (
                                len({cell_1, cell_2, cell_3}) != 3
                            ):  # check distinct cells
                                continue

                            triple_candidates_union = CellsGroup(
                                {cell_1, cell_2, cell_3}
                            ).get_candidates_union()

                            candidates_in_other_cells_in_rcs = CellsGroup(
                                empty_cells_in_rcs.difference({cell_1, cell_2, cell_3})
                            ).get_candidates_union()

                            if (
                                len(
                                    triple_candidates_union.difference(
